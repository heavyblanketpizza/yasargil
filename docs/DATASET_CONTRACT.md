...
The distinction matters in SOSpine. Tool-tip annotations originate from manual annotation; the released boxes were computed from those annotations. Importing a computed box does not make its geometry manual ground truth. Rolling needle-driver tip/base labels into a positive instrument-class statement is also an explicit mapping. The source has no persistent object-track identities from which to derive a verified trajectory automatically.
...
|---|---|
| Model-generated questions or answers | Supported only through a byte-verified teacher adapter, currently `ollama-evidence-v1`, plus actual applicable human review, eligibility and split gates. Unknown adapters remain blocked. |
