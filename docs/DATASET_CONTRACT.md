...
**Updated 2026-09-19. Status: v2 source import, bounded Qwen–MedGemma enhancement, teacher verification and export foundations are implemented.** The runtime retains model calls and evidence, creates a pending human-review packet, and projects messages with a receipt. Completed-review ingestion, actual expert review and downstream training experiments remain pending. Schema fields for broader workflows do not imply that every corresponding adapter exists. See [Enhancement](ENHANCEMENT.md) for the active loop and [Data sources](DATA_SOURCES.md) for source acquisition and repository exclusions.

The active schema is [enhancement-record.schema.json](../schemas/enhancement-record.schema.json). Runtime code lives in [src/yasargil](../src/yasargil/). The CLI exposes `models`, `enhance-sospine`, `import-sospine`, `validate`, `review-packet`, and `export`; use the locked environment through `uv sync --frozen` and `uv run yasargil`.
...
The deterministic baseline accepts 1–32 unique, increasing release-frame indices for one case. It records `explicit_indices` and re-expresses positive grasper/needle-driver labels from exact tool-tip rows; missing labels do not become absence claims. Computed box rows retain their original origin. This baseline performs no model inference or action interpretation. The separate enhancement path starts with a uniform sample, then optionally reinspects requested intervals, using Qwen and MedGemma to propose evidence-linked descriptions and questions. Both paths leave claims pending and the partition unassigned.
...
The distinction matters in SOSpine. The original visual annotations are CSV labels and coordinates for manually annotated instrument/durotomy points, plus boxes computed from those annotations. They are not prewritten descriptive prose. “Exact” preservation refers to the original row values and locations, not a guarantee that the labels are correct. Qwen writes new descriptive proposals, and MedGemma observes, reviews and revises them in the bounded enhancement loop. Importing a computed box does not make its geometry manual ground truth. Rolling needle-driver tip/base labels into a positive instrument-class statement is also an explicit mapping. The source has no persistent object-track identities from which to derive a verified trajectory automatically.
...
The baseline records `explicit_indices`. The bounded `enhance-sospine` selector records `hybrid` with implementation `qwen-medgemma-inspection-v1`: an initial deterministic uniform sample followed by bounded selection from model-requested intervals. Its plan retains available release indices, configuration and initial selection; later rounds record selected indices and stop reason. Qwen performs targeted local image reinspection. This bounded loop does not use native video processing, external search or a TimeLens2 adapter. The separate [complete-video selection](SMART_FRAME_SELECTION.md) and [frame annotation](FRAME_ANNOTATION.md) workflows do use native video, but their outputs are separate draft artifacts rather than this v2 archive/export adapter. Different selection strategies remain different experimental interventions; preserve an independent random reference sample when estimating quality so selected difficult cases do not masquerade as the natural distribution.
