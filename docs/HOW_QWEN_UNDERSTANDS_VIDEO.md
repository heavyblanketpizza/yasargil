...
Fine, Ollama is for noobs and I always liked llama.cpp better anyway. Then the timestamps in its responses ran past the end of test clips with llama.cpp's default vision handling. To debug that I had to learn what 'video-native' means so what does that mean? **ELI5**:
